...
set_node_memory_limit() {
    ...
    # 检查是否存在推荐内存方案并应用
    local recommendation_file="./build/recommendations/large_recommendation.txt"
...
build_complete_app() {
    ...
    # 构建完整应用
    log_info "执行完整应用构建..."
    node --no-warnings --no-compilation-cache ./node_modules/vite/bin/vite.js build --config build/configs/vite/vite.complete-app.config.ts --minify false --mode development --ssrManifest false
    ...
    if [ $exit_code -eq 0 ]; then
        log_success "完整应用构建成功!"
        log_info "构建耗时: ${build_duration}秒"
        log_info "构建时可用内存: ${available_memory}MB"
        log_info "分配给node进程的内存: ${memory_limit}MB"
        
        # 生成推荐内存构建方案
        generate_memory_recommendation "complete-app" $available_memory $memory_limit $build_duration
        ...
        log_error "错误: 完整应用构建失败 (退出码: $exit_code)"
        log_info "构建耗时: ${build_duration}秒"
        log_info "构建时可用内存: ${available_memory}MB"
        log_info "分配给node进程的内存: ${memory_limit}MB"
