...
test_component() {
    ...
    if ! [[ "$min_memory_limit" =~ ^[0-9]+$ ]] || [ "$min_memory_limit" -lt 50 ] || [ "$min_memory_limit" -gt 1100 ]; then
        echo "无法确定 ${name} 组件的最小内存限制: $min_memory_limit"
        return 1
    fi
    ...
    echo "${name} 组件的推荐内存为: ${recommended_memory}MB"
    # 返回推荐内存和最小内存限制，用逗号分隔
...
result=$(test_component "基础组件" "build/configs/vite/vite.shared-components.config.ts")
echo "结果: $result"