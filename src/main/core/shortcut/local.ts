...
const localShortcut = {
  register: (win: string | Electron.BrowserWindow, shortcut: Electron.Accelerator, func: () => void): void => {
  ...
  registerAll: (
    ...
    const instances =
      wins.length === 0
        ? getAllWin()
        : (wins
            .map((win) => (typeof win === 'string' ? getWin(win) : win))
            .filter(Boolean) as Electron.BrowserWindow[]);

    instances.forEach((instance) => {
      localShortcutModule.register(instance!, shortcuts, func);
    });
  },
  unRegister: (win: string | Electron.BrowserWindow, shortcut: Electron.Accelerator): void => {
    const instance = typeof win === 'string' ? getWin(win) : win;
  ...
  isRegistered: (win: string | Electron.BrowserWindow, shortcut: Electron.Accelerator): boolean => {
    const instance = typeof win === 'string' ? getWin(win) : win;
