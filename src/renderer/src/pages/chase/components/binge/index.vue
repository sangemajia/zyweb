    ...
    <div class="main">
      <t-row :gutter="[16, 16]" style="margin: 0">
        <t-col
          ...
          :key="item['id']"
          class="card"
          @click="handlePlayEvent(item)"
        >
          <div class="card-close" @click.stop="handleRemoveEvent(item)">
            <delete-icon />
          </div>
          <div class="card-main">
            ...
            >
              <template #overlayContent>
                <div class="op">
                  <div class="op-box">
                    <span>{{
                      item?.['relateSite']?.['name'] ? item['relateSite']['name'] : $t('pages.chase.sourceDeleted')
                    }}</span>
                  </div>
                </div>
              </template>
          ...
          <div class="card-footer">
            ...
            <p class="card-footer-desc tiles-item_desc_row text-hide">
              {{ item['videoRemarks'] ? item['videoRemarks'] : '' }}
            </p>
          </div>
        </t-col>
      </t-row>
...
import { useBingeSetup } from './bingeSetup';
