...
test_component() {
    ...
    if ! [[ "$min_memory_limit" =~ ^[0-9]+$ ]] || [ "$min_memory_limit" -lt 50 ] || [ "$min_memory_limit" -gt 1100 ]; then
        log_error "无法确定 ${name} 组件的最小内存限制: $min_memory_limit"
        return 1
    fi
    ...
    log_success "${name} 组件的推荐内存为: ${recommended_memory}MB"
    # 返回推荐内存和最小内存限制，用逗号分隔
...
result=$(test_component "基础组件" "build/configs/vite/vite.shared-components.config.ts" 2>&1)
echo "完整结果: $result"
echo "只保留数字和逗号: $(echo "$result" | grep -o '[0-9,]*$')"